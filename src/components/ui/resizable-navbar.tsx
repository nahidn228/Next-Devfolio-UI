...
export const Navbar = ({ children, className }: NavbarProps) => {
  ...
  return (
    <motion.div
      ref={ref}
      className={cn("stick inset-x-0 top-0 z-[9999]   w-full", className)}
    >
...
export const NavBody = ({ children, className, visible }: NavBodyProps) => {
  return (
    <motion.div
      ...
      className={cn(
        "relative z-[999] mx-auto hidden w-full container flex-row items-center justify-between self-start rounded-full bg-white border border-primary/20 dark:bg-neutral-950 px-4  py-2 lg:flex ",
      ...
      {items.map((item, idx) => (
        ...
        >
          {hovered === idx && (
            <motion.div
              layoutId="hovered"
              className="absolute inset-0 h-full w-full rounded-full bg-primary/30 dark:bg-primary/20"
            />
          )}
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        visible && "bg-primary/20 dark:bg-neutral-950/80",
        className
      )}
    >
      {children}
